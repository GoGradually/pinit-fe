...
- [ ] 일정 상세 페이지 (`ScheduleDetailPage`)
    ...
    - 하단 액션 버튼(고정 바 또는 본문 하단)
        - “시작”, “일시중지”, “완료”, “취소”, “수정”, “삭제”
- [ ] 일정 수정 모달 (`ScheduleEditModal`)
    - 진입: 상세 화면에서 “수정” 버튼 클릭 시 동일 모달 오버레이로 열림
    - `ScheduleForm` create/edit 공통 레이아웃을 그대로 사용(입력/검증/버튼이 동일)
    - 모달 상단 헤더:
        - 좌: 닫기
        - 중앙: “일정 수정”
        - 우: 저장 또는 설정(옵션)
...
### 세부 계획
- [ ] 일정 추가 모달 (`ScheduleCreateModal`)
    - 진입: 하단 중앙 탭 “추가” 버튼을 누르면 전체 화면 모달로 등장
    - 상단 헤더:
        - 좌: 닫기
        - 중앙: “일정 추가”
        - 우: 설정 아이콘(옵션)
    - 본문: `ScheduleForm` (create 모드) — 수정 모달과 동일 레이아웃/검증 재사용
...
### 세부 계획
- [ ] 의존성 섹션 UX
    - “이전 일정 추가하기” / “이후 일정 추가하기” 버튼을 누르면 단일 캘린더 모달을 띄워 날짜 선택
    - 날짜 확정 시, 해당 날짜 일정을 `/schedules?date=YYYY-MM-DD`로 조회(이미 로드된 목록이 있으면 캐시 재사용)
    - 모달 내 일정 목록에서 하나를 선택하면 “이전” 또는 “이후” 슬롯에 추가
        - 동일 일정은 이전/이후 중 하나만 택할 수 있도록 즉시 검증
- [ ] 프론트 검증
    - 자기 자신 일정은 후보에서 제외
...
- [ ] 전역 미니 플레이어 바
    - `IN_PROGRESS` 또는 `SUSPENDED` 일정이 있으면 `AppShell` 하단 탭 바로 위에 “미니 플레이어” 노출
    - `useActiveSchedule` 훅: 이미 로딩된 일정 리스트/상세 데이터를 우선 캐시 재사용하고, 필요 시 `/schedules?date=...` 또는 `/schedules/{id}`로 보강
    - 미니 플레이어 내 컨트롤은 `useScheduleActions` 로직을 공유해 시작/일시중지/완료/취소를 제공
    - 상태 변경 시 목록/상세/미니 플레이어 간 데이터 동기화 및 캐시 무효화 전략 정의
- [ ] 진행 중 일정 표시
    - 미니 플레이어 외에도, 일정 목록 탭 상단/헤더에 “현재 진행 중: XX” 배너/칩 표시 옵션 유지
- [ ] 동시 실행 제약과 UX
    - 백엔드가 “이미 진행 중인 일정이 있음” 에러를 반환할 경우 토스트/배너로 안내
    - `IN_PROGRESS` 일정이 존재하면 다른 일정의 “시작” 버튼 비활성화 또는 툴팁 처리
